...
func TestMail(t *testing.T) {
	config := `{"username":"pearcomms@gmail.com","password":"pearcomms","host":"smtp.gmail.com","port":587}`
	mail := NewEMail(config)
	if mail.Username != "pearcomms@gmail.com" {
		t.Fatal("email parse get username error")
	}
	if mail.Password != "pearcomms" {
