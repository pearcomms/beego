...
func TestUrlFor2(t *testing.T) {
	...
	if handler.URLFor("TestController.GetURL", ":username", "pearcomms") != "/v1/pearcomms/edit" {
		Info(handler.URLFor("TestController.GetURL"))
		t.Errorf("TestController.List must equal to /v1/pearcomms/edit")
	}
...
func TestPostFunc(t *testing.T) {
	r, _ := http.NewRequest("POST", "/pearcomms", nil)
	w := httptest.NewRecorder()

	handler := NewControllerRegister()
	handler.Add("/:name", &TestController{})
	handler.ServeHTTP(w, r)
	if w.Body.String() != "pearcomms" {
		t.Errorf("post func should pearcomms")
	}
}
